...
(function () {
  ...
  var CATEGORY_ORDER = ["Starters", "Mains", "Desserts", "Drinks"];
  ...
  function buildDish(item, variant) {
    ...
    head.appendChild(make("p", "dish-price", money.format(item.price)));
    body.appendChild(head);
    body.appendChild(make("p", "dish-desc", item.description));

    if (item.tags && item.tags.length) {
      var tags = make("ul", "tags");
      tags.setAttribute("aria-label", "Dietary information");
  ...
  function showError() {
    var message = make(
      ...
      "The menu could not load. If you opened this file straight from your computer, serve the folder " +
        "with a local server instead (see the README). Otherwise, check that data/menu.json exists and is valid."
    );
  ...
  function renderMenu(items) {
    ...
    function matches(item) {
      ...
      return state.tags.every(function (wanted) {
        /* A vegan dish is also vegetarian. */
        if (wanted === "Vegetarian") return tags.indexOf("Vegetarian") !== -1 || tags.indexOf("Vegan") !== -1;
    ...
    function apply() {
      ...
      count.textContent =
        visible === 0
          ? "No dishes match. Try fewer filters."
          : visible + (visible === 1 ? " dish" : " dishes") + (filtering ? " shown" : "");
    }
